...
def mkr(str):
    if str == "s":
        print("______________________________________________________ mkr s")
    elif str == "e":
        print("______________________________________________________ mkr e")
    else:
        print("______________________________________________________ mkr "+str+" s")
        print(str)
        print("______________________________________________________ mkr "+str+" e")


print("______________________________________________________ opening log s") 
time_s=time.time()
...
url_paths['1'] = '/'
@app.route(url_paths['1'])
def control1():
    return render_template("index.html")

url_paths['2'] = '/admin'
@app.route(url_paths['2'])
def control2():
    return render_template("admin.html")

url_paths['2_1'] = '/admin/board'
@app.route(url_paths['2_1'])
def control2_1():
    return render_template("board.html")

url_paths['2_2'] = '/admin/board/select'
@app.route(url_paths['2_2'])
def control2_2():
    return render_template("board_select.html")


url_paths['3'] = '/home'
@app.route(url_paths['3'])
def control3():
    return 'home'

url_paths['4'] = '/test'
@app.route(url_paths['4'])
def control4():
    return str(random.random())


url_paths['5'] = '/test_/<id>'
@app.route(url_paths['5'])
def control5(id):
    print(id)
    return str(id)

url_paths['6'] = '/test__'
@app.route(url_paths['6'])
def control6():
    return ''

def main():
    # app.run(host=host, debug=True, port=port)            # 테스트 서버 mode
    app.run(host=host, debug=False, port=port)          # 상용 서버 mode


# URL open 자동화 코드
# os.system('explorer "http://'+host+':'+str(port)+''+url_paths['5'])
os.system('explorer "http://'+host+':'+str(port)+''+url_paths['5'].replace("<id>",str(int(random.random()*100))))

if __name__ == '__main__':
    main()

# 이제 플라스크로 웹페이지간 라우팅을 할 수 있다
